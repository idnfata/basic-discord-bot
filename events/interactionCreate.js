  ...
  async execute(interaction, client) {
    ...
    if (interaction.customId == "open-ticket") {
      ...
      interaction.guild.channels.create(`ticket-${interaction.user.username}`, {
        ...
        interaction.reply({
          content: `Ticket berhasil dibuat! <#${c.id}>`,
          ephemeral: true
        });
        ...
        const row = new client.discord.MessageActionRow()
          .addComponents(
            ...
            .addOptions([{
                label: config.Category1,
                value: config.Category1,
                emoji: '💸',
              },
              {
                label: config.Category2,
                value: config.Category2,
                emoji: '🛠',
              },
              {
                label: config.Category3,
                value: config.Category3,
                emoji: '📑',
              },
            ]),
          );
    ...
    if (interaction.customId == "close-ticket") {
      ...
      const verif = await interaction.reply({
        content: 'Yakin ingin menutup ticket?',
        components: [row]
      });
      ...
      collector.on('collect', i => {
        if (i.customId == 'confirm-close') {
          interaction.editReply({
            content: `Ticket ditutup oleh <@!${interaction.user.id}>`,
            components: []
          });
